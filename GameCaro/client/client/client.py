...
class CaroGameClient:
    ...
    def toggle_theme(self):
        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                ...
                btn.config(bg=self.colors["BTN_COLOR"], fg=self.colors["BTN_TEXT"], activebackground=self.colors["BTN_ACTIVE"])
    ...
    def update_board_gui(self):
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                btn = self.buttons[r][c]
                btn.config(text=self.game_board[r][c])
                if self.last_move and (r, c) == (self.last_move.get('row'), self.last_move.get('col')):
